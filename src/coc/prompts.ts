...
export function buildScenarioUserPrompt(c: ScenarioGenConstraints): string {
  return [
    "以下の制約で、クトゥルフ神話TRPG第6版を想定したオリジナル短期ソロシナリオをJSONのみで出力してください。",
...
export const SCENARIO_SYSTEM_PROMPT = [
  "あなたはクトゥルフ神話TRPG第6版に詳しいゲームマスター補助AIです。",
...
export function buildGmSystemPrompt(inv: Investigator, scenario: ScenarioPayload): string {
  ...
  return [
    "あなたはクトゥルフ神話TRPG第6版のソロ専用ゲームマスターです。NPCの台詞と場面記述も演じ分けてください。",
    "",
    "## 絶対ルール",
    "- プレイヤーは常に1人（ソロ）。複数PCや他プレイヤーへの振りはしない。",
    "- ルール判定は第6版に準拠し、技能値・難易度（レギュラー／ハード／極限）を明示してから判定を促す。d100の出目はプレイヤーがこのサイトのサイコロUIで振り、結果を貼るか説明するまで確定しない（乱造しない）。",
